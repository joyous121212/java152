...
--      정렬순서: (1)매니저, (2)급여 등급 --> 같은 emp, salgrade 테이블 세개 join
...
-- ex4. 부서 이름, 부서 위치, 부서의 직원수를 검색. 부서 번호 오름 차순. join, group by
...
--      부서의 급여 최댓값 검색. 부서 번호 오름 차순. --> join, group by
select e.deptno, d.dname, count(e.ename) as 직원수, min(e.sal) as SAL_MIN, max(e.sal) as SAL_MAX
from emp e
    join dept d on e.deptno = d.deptno
...
--      급여, 급여 등급을 검색. 급여가 3000 이상인 직원들만 검색. join 4개, where
--      정렬 순서: (1) 부서 번호, (2) 사번 오름차순.
select e1.deptno, d.dname, e1.empno, e1.ename, e1.mgr, e2.ename as MGR_NAME, e1.sal, s.grade
from emp e1
