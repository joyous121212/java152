 ...
 * );
 * 
 * 컬럼의 데이터타입으로 사용되는 키워드 데이터베이스 종류에 따라서 다름.
 * 오라클 데이터 타입: number, varchar2(variable characters), date, timestamp, 
 *                     clob(character large object), blob(binary(mp3, mp4, jpg, ...) large object, 사용x 권장), ...
 */
...
-- 타입이 timestamp 이면 default가 systimestamp.
...
--> tdate에는 null 대신 기본값 sysdate(현재 시간)가 삽입 됨.
...
values (1003, 'Johne Doe', 'hgd@itwill.com');
--> 실패: unique 제약조건 위배

insert into ex_emp1 (eno, ename, age)
values (1004, 'Scott', -1);
--> 실패: chech (age >= 0) 제약조건 위배
