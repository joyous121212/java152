 ...
 */
 ...
 -- select nvl(comm, 0), sum(comm) from emp; (X)
...
select job, count(job) as COUNT, max(sal) as MAX, min(sal) as MIN, round(avg(sal), 2) as AVG_SAL
from emp
group by job
order by job;
...
select to_char(hiredate, 'YYYY') as YEAR, count(hiredate) as COUNT
from emp
group by to_char(hiredate, 'YYYY')
order by YEAR;

-- Ex. 부서별 급여 평균 검색. 급여 평균이 2000 이상인 부서만 검색. --> having
select deptno, round(avg(sal), 2) as AVG_SAL
from emp
group by deptno
having avg(sal) >= 2000
...
-- Ex. mgr 컬럼이 null이 아닌 직원들 중에서 부서별 급여 평균을 검색. --> where
--     정렬 순서: 부서번호 오름차순.
select deptno, round(avg(sal), 2) as AVG_SAL
from emp 
